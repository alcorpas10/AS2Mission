...
tmux send-keys "ros2 launch monitor execution_monitor.py drone_id:=0 config_file:=config/config_prueba_swarm_3.yaml" C-m 

tmux new-window -t $SESSION:2 -n 'Monitor 1'
tmux send-keys "ros2 launch monitor execution_monitor.py drone_id:=1 config_file:=config/config_prueba_swarm_3.yaml" C-m

tmux new-window -t $SESSION:3 -n 'Monitor 2'
tmux send-keys "ros2 launch monitor execution_monitor.py drone_id:=2 config_file:=config/config_prueba_swarm_3.yaml" C-m
...
tmux send-keys "ros2 launch viewer viewer.py config_file:=config/config_prueba_swarm_3.yaml" C-m

tmux new-window -t $SESSION:6 -n 'Mission Transmitter 0'
tmux send-keys "python3 mission_transmitter.py 3 drone_sim_" C-m
